...
public class TouchGestureDetector {
    ...
    private class OnTouchGestureListenerProxy implements IOnTouchGestureListener {
        ...
        public void onPointerDown(MotionEvent e) {
            mListener.onPointerDown(e);
        }

        @Override
        public void onPointerMove(MotionEvent e) {
            mListener.onPointerMove(e);
        }
        ...
        @Override
        public boolean onScale(ScaleGestureDetectorApi27 detector, MotionEvent e) {
            return mListener.onScale(detector, e);
        }

        @Override
        public boolean onScaleBegin(ScaleGestureDetectorApi27 detector, MotionEvent e) {
            mHasScaled = true;
            if (mIsScrolling) {
                mIsScrolling = false;
                onScrollEnd(mLastScrollMotionEvent);
            }
            return mListener.onScaleBegin(detector,e);
        }

        @Override
        public void onScaleEnd(ScaleGestureDetectorApi27 detector, MotionEvent e) {
            mListener.onScaleEnd(detector,e);
        }
    }
