...
public class RockerView extends View {
    ...
    private OnAngleDistanceChangeListener mOnAngleDistanceChangeListener;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:// 移出区域
                ...
                if (mOnAngleDistanceChangeListener != null) {
                    mOnAngleDistanceChangeListener.OnAngleDistanceChange(0, 0);
                }
    ...
    private void callBack(double angle, float distance) {
        ...
        Log.d(TAG, "callBack：" + angle + "," + distance);

        if (Math.abs(distance - lastDistance) >= (baseDistance / mDistanceLevel)) {
            lastDistance = distance;
            mLevel = (int) (distance / (baseDistance / mDistanceLevel));

            if (null != mOnDistanceLevelListener) {
                mOnDistanceLevelListener.onDistanceLevel(mLevel);
            }
        ...
        if (mOnAngleDistanceChangeListener != null) {
            mOnAngleDistanceChangeListener.OnAngleDistanceChange(angle, mLevel);
        }
     ...
     * @return
     */
    public OnAngleDistanceChangeListener getOnAngleDistanceChangeListener() {
        return mOnAngleDistanceChangeListener;
    }

    public void setOnAngleDistanceChangeListener(OnAngleDistanceChangeListener onAngleDistanceChangeListener) {
        mOnAngleDistanceChangeListener = onAngleDistanceChangeListener;
    }
    ...
    public interface OnAngleDistanceChangeListener {
        void OnAngleDistanceChange(double angle, int level);
    }
